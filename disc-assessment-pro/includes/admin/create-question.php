...
class Create_Question extends DiSC_Admin_Base {
    ...
    public function render() {
        ...
        $this->render_form($question_id, $test_id, $question_text);
    }

    private function render_form($question_id, $test_id, $question_text) {
        ?>
        <div class="wrap">
            ...
            <form method="post">
                ...
                <div id="answers-container" style="margin-bottom: 20px;">
                    <?php $this->render_answers($question_id); ?>
                </div>
    ...
    private function render_answers($question_id) {
        global $wpdb;
        if ($question_id > 0) {
            $answers = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}disc_answers WHERE question_id = %d", $question_id));
            foreach ($answers as $answer) {
                $this->render_answer_fields($answer);
            }
        } else {
            $this->render_answer_fields();
        }
    }

    private function render_answer_fields($answer = null) {
        ?>
        <div class="answer">
            <input class="answer-text" type="text" name="answer_text[]" value="<?php echo $answer ? esc_attr($answer->answer_text) : ''; ?>" placeholder="Answer Text" required>
            <div class="answer-group">
                <label>Adapted Style</label>
                <input type="number" name="score_d_adapted[]" value="<?php echo $answer ? esc_attr($answer->score_d_adapted) : ''; ?>" placeholder="D" required>
                <input type="number" name="score_i_adapted[]" value="<?php echo $answer ? esc_attr($answer->score_i_adapted) : ''; ?>" placeholder="I" required>
                <input type="number" name="score_s_adapted[]" value="<?php echo $answer ? esc_attr($answer->score_s_adapted) : ''; ?>" placeholder="S" required>
                <input type="number" name="score_c_adapted[]" value="<?php echo $answer ? esc_attr($answer->score_c_adapted) : ''; ?>" placeholder="C" required>
            </div>
            <div class="answer-group">
                <label>Natural Style</label>
                <input type="number" name="score_d_natural[]" value="<?php echo $answer ? esc_attr($answer->score_d_natural) : ''; ?>" placeholder="D" required>
                <input type="number" name="score_i_natural[]" value="<?php echo $answer ? esc_attr($answer->score_i_natural) : ''; ?>" placeholder="I" required>
                <input type="number" name="score_s_natural[]" value="<?php echo $answer ? esc_attr($answer->score_s_natural) : ''; ?>" placeholder="S" required>
                <input type="number" name="score_c_natural[]" value="<?php echo $answer ? esc_attr($answer->score_c_natural) : ''; ?>" placeholder="C" required>
            </div>
